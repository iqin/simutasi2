    ...
    <div class="filter-section">
        ...
        <form method="get" class="d-flex align-items-center gap-2">
            <select name="jenis" class="form-control form-control-sm" onchange="this.form.submit()">
                ...
                <option value="mutasi_tetap" <?= (isset($filterJenis) && $filterJenis === 'mutasi_tetap') ? 'selected' : '' ?>>Mutasi</option>
                <option value="nota_dinas" <?= (isset($filterJenis) && $filterJenis === 'nota_dinas') ? 'selected' : '' ?>>Nota Dinas</option>
                <option value="perpanjangan_nota_dinas" <?= (isset($filterJenis) && $filterJenis === 'perpanjangan_nota_dinas') ? 'selected' : '' ?>>Perpanjangan Nota Dinas</option>
            </select>
            ...
            <?php if (!empty($usulan)): ?>
                <?php foreach ($usulan as $index => $row): ?>
                    <tr>
                        ...
                        <td class="text-wrap jenis-usulan-col">
                            <?php
                                $jenisMap = [
                                    'mutasi_tetap' => 'MUTASI',
                                ...
                                echo $jenisMap[$row['jenis_usulan']] ?? strtoupper($row['jenis_usulan']);
                            ?>
                        </td>                        
                        <td class="action-column">
                            ...
                            <?php if ($role !== 'dinas'): ?> <!-- Dinas tidak bisa edit & hapus -->
                                ...
                                <?php elseif ($row['status'] === '02' && $row['status_usulan_cabdin'] === 'TdkLengkap'): ?>
                                    ...
                                    <a href="javascript:void(0)" class="btn btn-danger btn-sm-custom" 
                                    onclick="startRevisi('<?= $row['nomor_usulan'] ?>', <?= htmlspecialchars(json_encode($row['catatan_revisi'] ?? '')) ?>)">
                                ...
                                <?php elseif ($row['status'] >= '07' && ($role === 'admin' || $role === 'kabid')): ?>
                                    <!-- Status 07 ke atas: Hanya Admin/Kabid yang Bisa Dihapus -->
                                    <a href="#" class="btn btn-danger btn-sm-custom" onclick="confirmDelete('/usulan/delete/<?= $row['id'] ?>')">
                                        <i class="fas fa-trash-alt"></i>
                                    </a>
                                <?php endif; ?>
...
<div id="detailData" class="mt-4" style="display: none;">
    ...
    <table class="table table-bordered">
        ...
        <tr>
            <th>Jenis Usulan</th>
            <td id="detailJenisUsulan"></td>
        </tr>      
        <input type="hidden" id="hiddenJenisUsulan">     
        <tr>
            <th>Nama Guru</th>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        document.getElementById('detailJenisUsulan').textContent = jenisMap[row.jenis_usulan] || row.jenis_usulan;
        document.getElementById('hiddenJenisUsulan').value = row.jenis_usulan; // simpan jenis_usulan asli
    ...
    function startRevisi(nomorUsulan, catatanRevisi) {
        let catatanText = catatanRevisi 
            ? `<div class='alert alert-warning text-left'>
                <strong>Catatan Verifikasi Dokumen:</strong><br>${catatanRevisi}
            </div><br>` 
            : '';

        Swal.fire({
            title: 'Mulai Revisi Usulan?',
            html: catatanText +
                "Usulan ini berstatus <b>Tidak Lengkap</b>.<br><br>" +
                "Jika Anda lanjut revisi, Status usulan dikembalikan ke <b>01</b> (awal).<br><br>" +
                "<span class='text-danger font-weight-bold'>Proses ini tidak dapat dibatalkan!</span>",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Ya, Mulai Revisi',
            cancelButtonText: 'Batal'
        }).then((result) => {
            if (result.isConfirmed) {
                window.location.href = "/revisi_usulan/startRevisi/" + nomorUsulan;
            }
        });
    }
...
/*
    function submitRevisiForm(nomorUsulan) {
        Swal.fire({
            ...
            text: "Anda akan melakukan revisi berkas usulan. Data lama akan dihapus!",
            icon: 'warning',
            showCancelButton: true,
...
function showBerkasModal(nomorUsulan) {
    ...
    fetch(`/usulan/getDriveLinks/${nomorUsulan}`)
        ...
        .then(responseData => {
            ...
            // Ambil jenis_usulan dari hidden field (diset di showDetail)
            let jenisUsulan = document.getElementById('hiddenJenisUsulan').value || 'mutasi_tetap';


            // Default (Mutasi Tetap → 21 berkas)
            let berkasLabels = [
                "Surat Pengantar dari Cabang Dinas Asal",                           // 0
                "Surat Pengantar dari Kepala Sekolah",                              // 1
                "Surat Permohonan Pindah Tugas Bermaterai (Ditujukan Untuk Kepala Dinas)", // 2
                "Surat Permohonan Pindah Tugas Bermaterai (Ditujukan Untuk Kepala BKA)",   // 3
                "Surat Permohonan Pindah Tugas Bermaterai (Ditujukan Untuk Gubernur cq Sekda Aceh)", // 4
                "Rekomendasi Kepala Sekolah Melepas Lengkap dengan Analisis",       // 5
                "Rekomendasi dari Pengawas Sekolah Melepas",                        // 6
                "Rekomendasi Melepas dari Kepala Cabang Dinas Kab/Kota",            // 7
                "Rekomendasi Kepala Sekolah Menerima Lengkap dengan Analisis",      // 8
                "Rekomendasi dari Pengawas Sekolah Menerima",                       // 9
                "Rekomendasi Menerima dari Kepala Cabang Dinas Kab/Kota",           // 10
                "Analisis Jabatan (Anjab) dari Sekolah Melepas dan Sekolah Menerima", // 11
                "Surat Formasi GTK dari Sekolah Asal",                              // 12
                "Foto Copy SK 80% dan SK Terakhir di Legalisir",                    // 13
                "Foto Copy Karpeg dilegalisir",                                     // 14
                "Surat Keterangan tidak Pernah di Jatuhi Hukuman Disiplin",         // 15
                "Surat Keterangan Bebas Temuan Inspektorat (Optional)",             // 16
                "Surat Keterangan Bebas Tugas Belajar/Izin Belajar",                // 17
                "Daftar Riwayat Hidup/ Riwayat Pekerjaan",                          // 18
                "Surat Tugas Suami dan Foto Copy Buku Nikah (Optional)",            // 19
                "SKP 2 Tahun Terakhir"                                              // 20 (baru)
            ];
            ...
            if (jenisUsulan === 'nota_dinas') {
                ...
                activeIndexes = [0,2,5,6,7,8,9,10,13,17,18,19]; // 12 indeks
            ...
            responseData.data.forEach((berkas, index) => {
                if (!activeIndexes.includes(index)) return; // skip index non-aktif
